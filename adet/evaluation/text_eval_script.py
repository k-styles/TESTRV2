...
def validate_data(gtFilePath, submFilePath, evaluationParams):
    ...
    for k in gt:
        #print(f"Here is a k: {k} | subm: {subm} | gt: {gt} | evaluationParams: {evaluationParams}")
...
def evaluate_method(gtFilePath, submFilePath, evaluationParams):
    ...
    for resFile in gt:
        ...
        for n in range(len(pointsList)):
            ...
            if evaluationParams['LTRB']:
                ...
                #print("GTPOL (LTRB is True)", gtPol)
            else:
                gtPol = polygon_from_points(points)
                #print("GTPOL (LTRB is False)", gtPol)
            gtPols.append(gtPol)
        ...
        if resFile in subm:
            ...
            #print(f"POINTSLIST: {pointsList}")
            for n in range(len(pointsList)):
                points = pointsList[n]
                transcription = transcriptionsList[n]
                #print(f"ARE THE POINTS COMING: {points}")
                if evaluationParams['LTRB']:
                    detRect = Rectangle(*points)
                    detPol = rectangle_to_polygon(detRect)
                else:                    
                    detPol = polygon_from_points(points)
                #print(f"THIS IS DETPOL: {detPol}")
        ...
        #print(f"numGlobalCareDet : {numGlobalCareDet} | numDetCare :: {numDetCare}")
    ...
    #print(f"numGlobalCareGt : {numGlobalCareGt} | numGtCare :: {numGtCare}")
    methodRecall = 0 if numGlobalCareGt == 0 else float(matchedSum)/numGlobalCareGt
    #print(f"numGlobalCareDet : {numGlobalCareDet}")
